...
export default function WhatsAppCTA() {
  ...
  return (
    ...
    >
      {/* Expanded card */}
      <div
        className="rounded-2xl border overflow-hidden shadow-2xl shadow-black/40 transition-all duration-300"
        style={{
          width: expanded && visible ? '280px' : '0px',
          opacity: expanded && visible ? 1 : 0,
          pointerEvents: expanded && visible ? 'auto' : 'none',
          borderColor: 'hsl(var(--border))',
          backgroundColor: 'hsl(var(--background-light))',
          transform: expanded && visible ? 'translateY(0) scale(1)' : 'translateY(8px) scale(0.95)',
        }}
      >
        {/* Green header */}
        <div className="flex items-center justify-between px-4 py-3"
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-semibold text-white">Learning Group</span>
          </div>
          <button onClick={dismiss} className="text-white/80 hover:text-white text-lg leading-none" aria-label="Close">×</button>
        </div>
        ...
        <div className="p-4">
          ...
          <div className="flex flex-col gap-2">
            <a
              ...
              className="flex items-center justify-center gap-2 py-2.5 px-4 rounded-lg text-sm font-semibold text-white transition-opacity hover:opacity-90"
              style={{ backgroundColor: '#25D366' }}
              onClick={dismiss}
            >
      ...
      {visible && (
        <button
          ...
          className="w-14 h-14 rounded-full flex items-center justify-center shadow-2xl shadow-black/40 transition-transform hover:scale-110 active:scale-95"
          style={{ backgroundColor: '#25D366' }}
        >
